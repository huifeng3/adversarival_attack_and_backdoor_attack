...
import torchvision
from modle import PreActResNet18
# from networks.models import Denormalizer, NetC_MNIST, Normalizer  #这个地方也是一样，Normalizer可以看源码，Denormalize可以按源码反着写
from torch import nn
from torch.utils.tensorboard import SummaryWriter
from torchvision.transforms import RandomErasing
# from utils.dataloader import PostTensorTransform, get_dataloader
from utils import progress_bar, get_dataloader, Denormalizer, PostTensorTransform
...
def train(netC, optimizerC, schedulerC, train_dl, noise_grid, identity_grid, tf_writer, epoch, opt):
    ...
    for batch_idx, (inputs, targets) in enumerate(train_dl):
        ...
        bs = inputs.shape[0]   #样本总数

        # Create backdoor data!!!!!
        num_bd = int(bs * rate_bd)  #后门样本数
        num_cross = int(num_bd * opt.cross_ratio)   #噪声模式下的样本数
        grid_temps = (identity_grid + opt.s * noise_grid / opt.input_height) * opt.grid_rescale
        #除以height是使得每个像素点的移动控制在1格左右，至少不会很大
        ...
        #repeat函数进行各个维度复制，repeat里面的每一维的数字对应对应维度复制的次数，为1时不复制，若参数的个数大于原来的维度数，
        # 则先像最外层维度扩展一维后再复制维度
        grid_temps2 = torch.clamp(grid_temps2, -1, 1)    #将范围剪切到-1到1之间

        inputs_bd = F.grid_sample(inputs[:num_bd], grid_temps.repeat(num_bd, 1, 1, 1), align_corners=True)  #grid_sample
        # 是一个难点，它其实就是根据grid网格坐标来控制像素的位置，是扭曲的关键
        if opt.attack_mode == "all2one":
            targets_bd = torch.ones_like(targets[:num_bd]) * opt.target_label  #ones_like就是生成与targets形状相同的全1张量
        if opt.attack_mode == "all2all":
            targets_bd = torch.remainder(targets[:num_bd] + 1, opt.num_classes)  #返回前者除以后者的余数组成的张量
        ...
        total_inputs = transforms(total_inputs)  #这里是数据增强
        ...
        total_sample += bs
        total_loss_ce += loss_ce.detach()   #detach剥离loss中的梯度回传
        ...
        if not batch_idx % 50:  #每50个batch保存一次
...
def eval(
    ...
    for batch_idx, (inputs, targets) in enumerate(test_dl):
        with torch.no_grad():
            ...
            #注意这里，是将所有的输入都进行扭曲，也就是说即是训练的时候只让一部分样本进行了后门植入，最终模型训练的目的还是让模型都所有的样本都
            #可以触发后门
            if opt.attack_mode == "all2one":
    ...
    if acc_clean > best_clean_acc or (acc_clean > best_clean_acc - 0.1 and acc_bd > best_bd_acc):
        ...
        state_dict = {
            ...
            "epoch_current": epoch+1,   #这里用epoch+1比较好，因为测试搞完了说明该epoch就搞完了，下次就从epoch+1开始搞
...
def main():
    ...
    if opt.continue_training:    #从已有的地方开始训练，似乎默认是false
        ...
        # Prepare grid
        ins = torch.rand(1, 2, opt.k, opt.k) * 2 - 1   #rand是从[0,1)中均匀分布的采样数据
        ins = ins / torch.mean(torch.abs(ins))   #Normalize是保证产生的扭曲不会普遍过大或者过小
        noise_grid = (
            F.interpolate(ins, size=opt.input_height, mode="bicubic", align_corners=True)  #upsample被淘汰了，现在使用interpolate，使用的是双三次插值
            .permute(0, 2, 3, 1)
            .to(opt.device)
        ) #其实就是根据双三次插值法将ins上采样成noise_grid，height和width相等，所以只用height就行了
        array1d = torch.linspace(-1, 1, steps=opt.input_height)  #有steps个等差数字构成的array
        x, y = torch.meshgrid(array1d, array1d, indexing = 'xy')
        identity_grid = torch.stack((x, y), 2)[None, ...].to(opt.device)  #返回的是图片中每个像素点的坐标的张量左上角是[-1,-1]，右下角是[1,1]
        #这里不知道为什么原文是(x,y)，这样的话后门图片就是中心翻转过来的了，不知道为何要这样

        shutil.rmtree(opt.ckpt_folder, ignore_errors=True)  #递归删除opt.ckpt_folder文件
        os.makedirs(opt.log_dir)
